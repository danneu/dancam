...
spec for swoop 1. Start dumb, get footage moving, harden later.
...
them. Two tracks run in parallel: a **mock Pi** (a small fake local server with
canned status + sample frames/clips) so app work never blocks on hardware, and
the **real Pi** firmware. Each app swoop should pass against the mock first.

- [ ] **Swoop 0 -- Bring-up + mock.** Real Pi: flash Raspberry Pi OS (64-bit) with
...
- [ ] **Swoop 1 -- Live preview on iPhone.** Pi serves `GET /v1/preview/live.mjpeg`
      ...
      *spike: confirm `NWConnection` Wi-Fi pinning + no-internet-AP / captive-probe
      handling behaves.*
- [ ] **Swoop 2 -- Recording control + live status.** Start/stop recording buttons
...
- [ ] **Swoop 3 -- SD card management.** Pi detects the card and surfaces issues
...
- [ ] **Swoop 4 -- Watch recorded clips.** Browse a clip list (`GET /v1/clips`),
...
- [ ] **Swoop 5 -- Time provenance.** `POST /v1/time` at handshake (the Pi has no
      RTC); "time unverified" UI until sync; timestamps on clips.
- [ ] **Swoop 6 -- Incident lock (manual).** A "save this moment" button: Pi
      ...
      later.*
- [ ] **Swoop 7 -- Connection robustness.** Persistent auto-rejoin
...
- [ ] **Swoop 8 -- CarPlay voice incident-mark.** App Intents "save that clip,"
      hands-free, with queue-and-flush on the cold path. *No entitlement needed --
      the highest-value, lowest-risk CarPlay piece.*
- [ ] **Swoop 9 -- CarPlay auto start/stop** on CarPlay connect/disconnect.
- [ ] **Swoop 10 -- CarPlay status panel** (Driving Task template). *Gated on the
      Apple entitlement; the product must be useful without it.*
- [ ] **Swoop 11 -- Export / share.** TS -> MP4 passthrough remux to Photos /
      AirDrop (export-only, off the playback path).
- [ ] **Swoop 12 -- Power-loss hardening for real.** Power-good GPIO + clean
