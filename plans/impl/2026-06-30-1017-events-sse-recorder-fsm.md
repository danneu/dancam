...
- [x] 3. camera child lifecycle protocol (session + segment events)
- [x] 4. fold events into state machines; recorder-driven live row; drop polls
- [x] 5. cross-cutting sweep
...
- Commit 3 teaches `CameraConfig::new` to infer `--rec-dir` from custom camera args so the Rust start-index scan and final `ClipMeta` construction use the same directory the child writes to in fake-camera integration tests and `DANCAM_CAMERA_CMD` overrides.
- Commit 4 uses a small capped event-stream reconnect backoff (1s, 2s, then 4s) behind `streamReconnectAttempt`; the plan required backoff reconnect behavior but did not prescribe exact delays.
- Commit 5 corrected the app ADR 10 related link to the landed raspi ADR filename (`10-2026-06-30-recorder-fsm-and-events-sse.md`), because commit 2 created the ADR on 2026-06-30 rather than the plan's earlier suggested 2026-06-29 date.
