...
systemd unit and deploy paths stay `deploy.sh`'s. The
[Pi setup runbook](../docs/setup/pi-runbook.md) covers bootstrap/verify/ops -- flash,
SSH, smoke-tests, the one-time manual AP PSK, the AP safe-flip procedure -- and is
kept in sync in the same change whenever the human-facing steps move.
