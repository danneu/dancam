...
verify_development_image() {
  local image_root=$1 image_id=$2 root_partuuid=$3 wifi_country=$4
  local persist_label=$5 data_label=$6 catalog package installed uid gid unit
  ...
  while IFS= read -r package; do
    installed=$(chroot "$image_root" /usr/bin/dpkg-query -W -f='${Version}' "$package") || \
      verify_die "development package is absent: $package"
    [ -n "$installed" ] || verify_die "development package version is empty: $package"
  done < <(jq -r '.system_common_package_catalog[] | select(.development) | .name' "$catalog")

  verify_file "$image_root/usr/local/bin/dancam"
  [ -x "$image_root/usr/local/bin/dancam" ] || verify_die 'service binary is not executable'
  cmp -s "$image_root/usr/local/lib/dancam/camera.py" "$SCRIPT_ROOT/raspi/camera/camera.py" || \
    verify_die 'camera owner does not match the tracked worktree source'
  cmp -s "$image_root/etc/systemd/system/dancam.service" "$SCRIPT_ROOT/raspi/dancam.service" || \
    verify_die 'service unit does not match the tracked worktree source'
  verify_symlink "$image_root/etc/systemd/system/multi-user.target.wants/dancam.service" \
    /etc/systemd/system/dancam.service
  verify_symlink "$image_root/etc/systemd/system/timers.target.wants/fstrim.timer" \
    /usr/lib/systemd/system/fstrim.timer
  ...
  verify_line "$image_root/etc/fstab" 'LABEL=bootfs /boot/firmware vfat defaults,noatime 0 2'
  verify_line "$image_root/etc/fstab" 'LABEL=rootfs / ext4 defaults,noatime,errors=remount-ro 0 1'
  verify_line "$image_root/etc/fstab" "LABEL=$persist_label /persist ext4 noatime,errors=remount-ro,nofail,x-systemd.device-timeout=10s 0 2"
  verify_line "$image_root/etc/fstab" "LABEL=$data_label /data ext4 noatime,errors=remount-ro,nofail,x-systemd.device-timeout=10s 0 2"
  verify_line "$image_root/etc/fstab" '/persist/journal /var/log/journal none bind,nofail 0 0'
  ...
  [ "$(jq -r .image_id "$image_root/boot/firmware/dancam/image.json")" = "$image_id" ] || \
    verify_die 'image marker identity is wrong'
  [ "$(jq -r .profile "$image_root/boot/firmware/dancam/image.json")" = development ] || \
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  [ "$#" -eq 7 ] || {
    echo 'usage: verify-image.sh PROFILE ROOT IMAGE_ID ROOT_PARTUUID WIFI_COUNTRY PERSIST_LABEL DATA_LABEL' >&2
    exit 64
  }
  profile=$1
  shift
  case "$profile" in
    production) verify_production_image "$@" ;;
    development) verify_development_image "$@" ;;
    *) verify_die "unknown image profile: $profile"; exit 64 ;;
  esac
  printf '%s image inspection passed\n' "$profile"
fi
