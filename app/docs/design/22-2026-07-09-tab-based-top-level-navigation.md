...
The app architecture retained the shell as the owner of global app chrome. The
[app connection design](../../../docs/design/app/connection.md) defines that chrome
as the current connection and recording dual-pill status strip. Adding a peer surface
must preserve the strip above every screen and keep the scene's composition in
`SceneDelegate`.
